...
var (
	...
	// ignoreResourceBindingFieldsKeepAnnotations compares binding annotations; use it where
	// the test knows what the annotations should be, such as an unscheduled binding whose
	// previous state the test itself set up.
	ignoreResourceBindingFieldsKeepAnnotations = []cmp.Option{
	...
	// ignoreResourceBindingFields additionally ignores annotations, for the assertions that
	// cannot predict them.
	ignoreResourceBindingFields = append(
		[]cmp.Option{ignoreObjectMetaAnnotationField},
		ignoreResourceBindingFieldsKeepAnnotations...,
	)
)
